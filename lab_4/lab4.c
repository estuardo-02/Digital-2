...
//*****************************************************************************
void setup(void);
int conversion_segs(int adc_val, int* decenas, int* unidades){
    *decenas = adc_val/10;
...
void main(void) {
    ...
    while(1){
        ...
        //iniciar secuencia para recuperar segundos
        I2C_Master_Start();
        I2C_Master_Write(0xD0); //direccion de RTC (escribo desde maestro)
        I2C_Master_Write(0x00); //direccion del registro del RTC DS3231 (segundos)
        I2C_Master_Stop();
        __delay_ms(20);
        I2C_Master_Start();
        I2C_Master_Write(0xD1); //direccion de RTC (ahora leo desde maestro)
        segundos = I2C_Master_Read(0); //con Acknowledge activado
        I2C_Master_Stop();
        __delay_ms(20);
